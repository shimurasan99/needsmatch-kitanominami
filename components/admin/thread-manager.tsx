...
export function ThreadManager({ initialThreads }: { initialThreads: MessengerThread[] }) {
  ...
  useEffect(() => {
    void fetchSharedState<MessengerThread[]>("threads").then((value) => value && setThreads(value)).catch(() => setMessage("共有データを読み込めませんでした。"));
  }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    const next = editingId === "new"
      ? [...threads, { id: `thread-${Date.now()}`, name: name.trim(), url: url.trim() }]
      : threads.map((thread) => thread.id === editingId ? { ...thread, name: name.trim(), url: url.trim() } : thread);
    try { await saveSharedState("threads", next); setThreads(next); setEditingId(null); setMessage("保存しました。"); }
  ...
  async function confirmDelete() {
    ...
    try { await saveSharedState("threads", next); setThreads(next); setDeleteId(null); setMessage("削除しました。"); }
