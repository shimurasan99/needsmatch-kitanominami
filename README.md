...
- `006_atomic_table_publication.sql`：テーブル割り公開の同時更新検査。保存版・公開版を同じDBトランザクション内で確認する公開用RPCを追加（既存の割り当てデータは変更しません）
...
公開は専用API `/api/meetings/[id]/table-assignment-publication` を使用し、DB内の保存済み内容だけを公開します。006を適用してから対応版をデプロイしてください。旧版の共有データAPIへの直接公開は428で拒否し、006未適用時も公開をエラーとして停止します。別の運営者が同じ月例会を保存・公開した場合は、再読み込みして内容を確認してください。
