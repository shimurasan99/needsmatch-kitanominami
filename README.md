...
| 公開テーブル割り（トップページ・会員ページ） | `shared_site_state` の `table-assignments` |
...
5. 保存後に「保存済みの内容を公開する」を押すと会員画面とログイン不要の閲覧ページへ反映されます。未保存の編集内容は公開されません。
...
対象月例会の「自動テーブル割り」で「急遽参加する会員を手動で追加」を開き、会員と追加先のテーブルを選択して追加します。既存の席順を変えずに末尾へ追加し、配置済みの会員は候補から除外します。新規登録直後に候補へ出ない場合は「会員名簿・出欠を更新」を押してください。名簿更新で編集中の配置はリセットされません。

「保存」で運営間に共有し、「保存済みの内容を公開する」で会員ページにも反映します。保存前は「直前の追加を取り消す」で最後の追加を取り消せます。この操作は出欠回答を変更しないため、未定・欠席の方は参加者管理でも参加へ変更してください。定員超過は警告されますが手動追加は可能です。手動追加時には、過去の同席重複や業種バランスを自動調整しません。
...
トップページの次回定例会カードにある「テーブル割を見る」は、該当定例会の `/table-assignments/[id]` を開きます。このページはログイン不要です。公開前は未公開メッセージを表示し、他の月のテーブルへ自動で切り替えません。公開情報は `/api/meetings/[id]/public-table-assignment` から取得し、氏名・業種/ゲスト会社・リーダー表示などの必要項目だけを返します。連絡先・会員番号・会員ID・下書きは返しません。会員専用ページ・運営編集・出欠回答・従来の集約APIの認証は維持します。公開ページは検索インデックス対象外ですが、URLを知っている方は閲覧できます。

## 自動テーブル割り

### 急な欠席への変更

各席の「欠席にする」から対象者を確認し、「欠席にして配置から外す」を押します。会員の出欠はその場で共有保存され、その方だけが編集中の配置から外れます。他の席順・テーブル数は変えません。配置自体はまだ未保存のため、続けて「保存」→「保存済みの内容を公開する」を押してください。確認画面でキャンセルした場合は何も変更しません。

ゲストは氏名と会社表示が参加者名簿に一意に一致する場合に出欠も保存します。テーブルだけに手入力した未登録ゲストは配置のみ変更し、その旨を表示します。同名・同会社などで対象を一意に確認できない場合は変更せずエラーを表示します。参加者管理でもゲストの参加／欠席を変更でき、欠席ゲストは参加人数・自動生成から除外されます。参加へ戻す場合は参加者管理で出欠を戻し、必要な席へ手動追加してください。

出欠保存に失敗した場合は配置を保持します。通信切断などで保存確認に失敗した場合は出欠のみ保存済みの可能性があるため、最新の出欠を確認して再実行してください。全員が欠席になり空席のみとなったテーブルも、マイグレーション `007_publish_empty_seating_tables.sql` 適用後は保存・公開できます（テーブル自体が0件の公開は不可）。

### ゲスト・テーブルの追加

「ゲスト・テーブルを手動で追加」から、ゲストの氏名・会社名（任意）・追加先を指定して追加できます。同じ氏名・会社名のゲストの二重追加は防止します。「Fテーブルを追加」などのボタンは、次の名前の空テーブルを末尾に増やします。既存の参加者は各席の移動先から新しいテーブルへ移動できます。既存配置の再生成は不要です。

空テーブルだけでも下書きとして保存できます。保存前なら直前のゲスト追加を取り消せます。直前に追加したテーブルの取り消しは、空の場合だけ可能です。変更後は「保存」、会員向けに反映する場合は「保存済みの内容を公開する」を押してください。

このゲスト入力はテーブル割り専用で、参加者名簿・出欠は変更しません。参加者名簿にも必要な場合は「参加者管理」にも登録してください。後から自動生成をやり直す場合は参加者管理の名簿が使われるため、手動追加したゲストを含めるには名簿への登録が必要です。
